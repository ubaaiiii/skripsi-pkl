...
<form class="form" method="post" id="form-admin">
  <?= csrf_field(); ?>
<input type="hidden" id="nomor_induk_real" name="nomor_induk_real" value="<?=isset($admin->nomor_induk)?$admin->nomor_induk:'';?>">
    ...
    <div class="col-lg-6 col-md-6 col-sm-12">
      <div class="col-12">
          <div class="form-label-group position-relative has-icon-left">
              <input type="number" <?=($tipe=='lihat')?('disabled'):('required');?> id="nomor_induk" class="form-control" name="nomor_induk" placeholder="Nomor Induk" value="<?=isset($admin->nomor_induk)?($admin->nomor_induk):('');?>">
      ...
      <div class="col-12">
          <div class="form-label-group position-relative has-icon-left">
              <input type="text" <?=($tipe=='lihat')?('disabled'):('required');?> id="nama" class="form-control" name="nama" placeholder="Nama Lengkap" value="<?=isset($admin->nama)?$admin->nama:'';?>">
              <div class="form-control-position">
          ...
          <div class="form-label-group position-relative">
              <ul class="list-unstyled mb-0">
                  <li class="d-inline-block mr-2">
                      <fieldset>
                          <div class="vs-radio-con">
                              <input <?=($tipe=='lihat')?('disabled'):('required');?> type="radio" name="jenis_kelamin" value="L" <?=isset($admin->jenis_kelamin)?(($admin->jenis_kelamin == 'L')?'checked':''):'';?>>
                              <span class="vs-radio">
                  ...
                  <li class="d-inline-block mr-2">
                      <fieldset>
                          <div class="vs-radio-con">
                              <input <?=($tipe=='lihat')?('disabled'):('required');?> type="radio" name="jenis_kelamin" value="P" <?=isset($admin->jenis_kelamin)?(($admin->jenis_kelamin == 'P')?'checked':''):'';?>>
                              <span class="vs-radio">
                                  <span class="vs-radio--border"></span>
                                  <span class="vs-radio--circle"></span>
                              </span>
      ...
      <div class="col-12 mt-2">
          <fieldset class="form-label-group position-relative has-icon-left">
              <textarea class="form-control" id="alamat" rows="3" name="alamat" <?=($tipe=='lihat')?('disabled'):('required');?> placeholder="Alamat"><?=isset($admin->alamat)?$admin->alamat:'';?></textarea>
              <div class="form-control-position">
        ...
        success: function(resp){
          ...
          if (resp !== "berhasil" && resp !== "exists") {
            ...
            if (resp == 'berhasil') {
              ...
              toastr.success("Berhasil menyimpan data admin", 'Success!', { "timeOut": 5000 });
              $('#large').modal('hide');
            } else {
